  ...
  return (
    <nav className="fixed top-0 w-full bg-white text-black p-4 z-50 shadow-md h-[90px]">
      <div className="container mx-auto flex justify-between items-center h-full">
        ...
        <ul
  className={`md:flex space-x-2 absolute md:static w-full md:w-auto top-16 left-0 md:flex-row flex-col md:space-x-2 space-y-2 md:space-y-0 p-2 md:p-0 transition-all duration-300 ${
...
{["Accueil", "Nissan", "Peugeot", "Foton", "Lassa", "Canon", "Blog", "Contact"].map((item) => (
  <li key={item} className="px-2 py-1 text-xs transition-all">
    <Link 
      ...
      className="hover:text-[#c3002f] hover:text-sm transition-all"
    >
      {item}
    </Link>
  </li>
